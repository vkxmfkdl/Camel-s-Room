        ...
        jqbar: function (options) {
            ...
            return this.each(function () {
                ...
                function animateProgressBar(progressBar) {
                    ...
                    if (level >  5) {
                        level =  5;
                        alert('max value cannot exceed  5 percent');
                    }
                    var w = settings.barLength * level /  5;

                    if (settings.orientation == 'h') {
                        progressBar.animate({ width: w }, {
                            duration: 2000,
                            step: function (currentWidth) {
                                var percent = parseInt(currentWidth / settings.barLength *  5);
                                if (isNaN(percent))
                                    percent = 0;
                        ...
                        var h = settings.barLength - settings.barLength * level /  5;
                        progressBar.animate({ top: h }, {
                            duration: 2000,
                            step: function (currentValue) {
                                var percent = parseInt((settings.barLength - parseInt(currentValue)) / settings.barLength *  5);
                                if (isNaN(percent))
                                    percent = 0;
